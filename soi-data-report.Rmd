...
author: "Lauren Parker, Community Legal Services"
date: "Last update: April 2, 2019"
output: 
  html_document:
    theme: flatly
    code_folding: hide
...
tenure <- read.csv("data/census/ACS_17_5YR_B25003_TENURE/ACS_17_5YR_B25003_with_ann_TENURE.csv", stringsAsFactors = FALSE, colClasses=c("Id2"="character")) %>%
  mutate(tract = Id2) %>%
  select(-Id, -Id2)
...
# Import Census Data
hisp <- read.csv("data/census/ACS_17_5YR_B03002_HISP/ACS_17_5YR_B03002_with_ann_HISP.csv", stringsAsFactors = FALSE)
pov <- read.csv("data/census/ACS_17_5YR_S1701_POVERTY/ACS_17_5YR_S1701_with_ann_POV.csv", stringsAsFactors = FALSE)
...
tracts <- st_read("T:/GIS Basemap/census/tl_2018_42_tract.shp", layer = "tl_2018_42_tract", stringsAsFactors = FALSE) %>%
  mutate(tract = GEOID) %>%
  select(tract, geometry)

# Import County Shapefile
counties <- st_read("T:/GIS Basemap/census/tl_2018_us_county.shp", layer = "tl_2018_us_county", stringsAsFactors = FALSE) %>%
...
zcta <- st_read("T:/GIS Basemap/census/Phila_ZCTA_WGS84.shp", layer = "Phila_ZCTA_WGS84", stringsAsFactors = FALSE)

# Import Median Gross Rent Data
rent <- read.csv("data/census/ACS_17_5YR_B25064_RENT/ACS_17_5YR_B25064_with_ann_RENT.csv", stringsAsFactors = FALSE) %>%
...
tract.neigh <- read.dbf("T:/SOI Discrimination/shps/Tracts_Neigh_Int.dbf", as.is = TRUE) %>%
  mutate(tract = GEOID) %>%
...
neigh <- st_read("T:/GIS Basemap/odp/Neighborhoods_Phila_WGS84.shp", layer = "Neighborhoods_Phila_WGS84", stringsAsFactors = FALSE) %>%
  mutate(neigh = MAPNAME) %>%
